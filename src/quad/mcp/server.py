"""QUAD MCP Server — FastMCP registration.

Relocated from ``quad.server`` so the layering is explicit:

* :mod:`quad.core`    — pure logic (no MCP, no client)
* :mod:`quad.mcp`     — this layer; registers tools with FastMCP
* :mod:`quad.client`  — Claude Code (and future) client provisioning

The :mod:`quad.server` module remains as a backward-compat shim.
"""
...
def cli() -> None:
    """Entrypoint for ``quad-server`` (Claude Code spawns this via stdio)."""
    mcp.run()
